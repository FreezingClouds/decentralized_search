...
class Location(object):
    ...
    def distance(self, location):
        return np.sqrt(np.square(location.x - self.x) + np.square(location.y - y))
...
class SwarmPoint(Agent):
    def __init__(self, x, y, alpha1, alpha2, alpha3):
        super(SwarmPoint, self).__init__(x, y, update_every_k_steps=1)
        self.alphas = (alpha1, alpha2, alpha3)
        return

    def move_one_step(self, map):
        x, y = self.curr_location
        map.num_swarm_points[x, y] -= 1
        if map.evader_detected:
            ...
            neighbor_values = [compute_weighted_sum(neighbor_location, map) for neighbor_location in neighbors]
    ...
    def compute_weighted_sum(self, location, map):
        return sum([p.location.distance(location) * self.alphas[i] for i, p in enumerate(map.pursuers)])
